...
function renderSel(){
  ...
  document.getElementById('selcount').textContent='Selected '+selected.length+' items';
  var z=document.getElementById('selZipBtn');if(z)z.disabled=selected.length===0;
}
function zipDown(){
  if(!selected.length){toast(false,'Select files first');return;}
...
function render(){
  ...
  if(dir){
    ...
    var bn=document.createElement('div');bn.className='name';bn.textContent='Up one level';
    var bs=document.createElement('div');bs.className='sub';bs.textContent=parent?('📁 '+parent):'📁 Root';
  ...
  if(!items.length){
    ...
    empty.innerHTML='<div class="big">🗂️</div><div style="font-weight:700;color:var(--ink)">'+(dir?'This folder is empty':'Nothing here yet')+'</div><div style="margin-top:6px">'+(dir?'':'Files appear here after an admin uploads them')+'</div>';
    g.appendChild(empty);
  }
  items.slice((page-1)*per,page*per).forEach(function(it){
    ...
    if(it.is_dir){
      ...
      var b=document.createElement('button');b.className='btn btn-ghost btn-sm';b.textContent='Open';
      ...
      var a=document.createElement('a');a.className='btn btn-primary btn-sm';a.href=P+'/d?p='+encodeURIComponent(it.path);a.textContent='↓ Download';
  ...
  document.getElementById('st-count').innerHTML='<b>'+fc+'</b><span>files</span>';
  document.getElementById('st-size').innerHTML='<b>'+fmt(USED)+'</b><span>used</span>';
  document.getElementById('st-quota').innerHTML='<b>'+fmt(free)+'</b><span>available</span>';
  ...
  if(ql)ql.textContent='Used '+fmt(USED)+' · Reserved '+fmt(RESERVED)+' · Cache '+fmt(PENDING)+' · Free '+fmt(free)+' · Total '+fmt(QUOTA);
  var pg=document.getElementById('pager');
  if(pg){
    if(items.length<=PAGE_SIZE){pg.style.display='none';}
    else{
      pg.style.display='flex';
      document.getElementById('pgInfo').textContent='Page '+page+' / '+pageCount+' of '+items.length+' items';
  ...
  var root=document.createElement('a');root.textContent='🏠 Root';root.href='javascript:void 0';root.onclick=function(){nav('');};cr.appendChild(root);
  if(dir){
    var acc='';
...
PUBLIC_HTML = """<section class="hero">
  <div class="hrow">
    <div class="brand">
      <div class="logo">📁</div>
      <div><h1>ShareHub</h1></div>
    </div>
  </div>
  <div class="acts">
    <a class="btn btn-white btn-sm" href="__PREFIX__/admin">🔐 Admin</a>
  </div>
  <div class="stats">
    <div class="stat" id="st-count"><b>-</b><span>files</span></div>
    <div class="stat" id="st-size"><b>-</b><span>total size</span></div>
    <div class="stat" id="st-quota"><b>-</b><span>capacity</span></div>
  </div>
...
<div class="pager" id="pager" style="display:none">
  <button class="btn btn-ghost btn-xs" id="pgPrev">← Prev</button>
  <span id="pgInfo"></span>
  <button class="btn btn-ghost btn-xs" id="pgNext">Next →</button>
  <span class="pg-jump">Go to <input id="pgJump" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="4" page</span><button class="btn btn-ghost btn-xs" id="pgGo">Go</button>
</div>
<div class="selbar" id="selbar" style="display:none">
  <span class="cnt" id="selcount">Selected 0 items</span>
  <button class="btn btn-ghost btn-xs" id="selAllBtn">Select all in folder</button>
  <button class="btn btn-primary btn-sm" id="selZipBtn">↓ Download ZIP</button>
  <button class="btn btn-ghost btn-sm" id="selClearBtn">Cancel</button>
</div>
"""
...
function batchDel(){
  ...
  if(!confirm('Delete selected '+selected.length+' items? This cannot be undone.'))return;
  ...
  fetch(P+'/admin/batch',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Deleted '+j.done+' items'+(j.failed&&j.failed.length?(', failed '+j.failed.length+' items'):''));
      if(j.failed&&j.failed.length)toast(false,j.failed.map(function(x){return x.path+':'+x.error;}).join('；'));
    }else{toast(false,j.error||'Delete failed');}
    selected=[];selMode=false;refresh();renderSel();
  }).catch(function(){toast(false,'Delete failed');});
}
...
async function runTask(t){
  try{
    ...
    for(var i=0;i<t.total;i++){
      ...
      else{fail(t,'Chunk '+i+' upload failed');return;}
    }
    if(t.status==='cancelled'||t.status==='paused')return;
    ...
    finish(t);
  }catch(e){fail(t,'Network error, retryable');}
}
...
function showDone(n){
  ...
  box.innerHTML='<div class="tdone">✓ All uploads complete · '+n+' files</div>';
...
function finalizePool(){
  ...
  if(!f&&!p){
    ...
    updateTasksum();
    renderTasks();
    toast(f?false:true,(n+' completed'+(f?('，'+f+' failed'):'')+(p?('，'+p+' paused'):'')));
  }
  refresh();
}
function finish(t){
  if(_batchSize<=1)toast(true,'Uploaded: '+t.name);
...
function buildTaskRow(t){
  ...
  if(t.status==='failed'){
    ...
    var rb=document.createElement('button');rb.className='btn btn-ghost btn-xs';rb.textContent='Retry';
    rb.onclick=function(){retryTask(t);};r.appendChild(rb);
  }else if(t.status==='waiting'){
    st.textContent='Waiting';
  }else if(t.status==='paused'){
    st.textContent='Paused '+pct+'%';
    var rs=document.createElement('button');rs.className='btn btn-add btn-xs';rs.textContent='Resume';
    rs.onclick=function(){resumeTask(t);};r.appendChild(rs);
  }else if(t.status==='done'){
    tb.classList.add('ok');
    st.className='st done';st.textContent='✓ Done';
  }else{
    st.textContent=pct+'% · '+fmt(t.uploaded);
    var pz=document.createElement('button');pz.className='btn btn-ghost btn-xs';pz.textContent='Pause';
    pz.onclick=function(){pauseTask(t);};r.appendChild(pz);
  }
  var cx=document.createElement('button');cx.className='btn btn-danger btn-xs';cx.textContent='Cancel';
...
function updateTasksum(){
  ...
  if(!queue.length){s.textContent='';}
    ...
    s.textContent=up?(' · uploading '+up+' files'):(' · queue '+queue.length+' files');}
}
function renderTaskPager(){
  ...
  document.getElementById('tpInfo').textContent='Tasks '+tPage+' / '+tPageCount+' of '+queue.length+' files';
...
function updateTaskProgress(t){
  ...
  var ps=r.querySelector('.ps');if(ps){
    var dc=0;for(var kk in t.done)dc++;
    ps.textContent=(t.path!==t.name?t.path+' · ':'')+fmt(t.size)+' · chunk '+dc+'/'+t.total+(t.err?(' · '+t.err):'');
  }
}
...
function renderBulk(){
  ...
  if(st){
    if(!queue.length){st.style.display='none';st.innerHTML='';}
      ...
      item('total','Total files',queue.length);
      item('done','Done',done);
      item('up','Uploading',up);
      item('wait','Waiting',wait);
      item('pause','Pause',pause);
      item('fail','Failed',fail);
    }
  }
}
function retryAll(){
...
function addBatch(batch,targetDir){
  ...
  fetch(P+'/api/files').then(function(r){return r.json();}).then(function(j){
    ...
    if(total>avail){
      toast(false,'Selected total '+fmt(total)+', available '+fmt(Math.max(0,avail))+', exceeds capacity, not added');
      return;
    }
...
function render(){
  ...
  if(!items.length){
    var empty=document.createElement('div');empty.className='empty';
    empty.innerHTML='<div class="big">📤</div><div style="font-weight:700;color:var(--ink)">'+(dir?'This folder is empty':'Nothing here yet')+'</div><div style="margin-top:6px">'+(dir?'':'Drag files/folders above, or click the buttons to upload')+'</div>';
    g.appendChild(empty);
  }
  items.slice((page-1)*per,page*per).forEach(function(it){
    var c=document.createElement('div');c.className='card file-card';
    ...
    rb.title=it.is_dir?'Rename folder':'Rename file';
    ...
    grp.appendChild(db);
    if(it.is_dir){
      var ob=document.createElement('button');ob.className='btn btn-ghost btn-xs';ob.textContent='Open';
      ...
      var a=document.createElement('a');a.className='btn btn-ghost btn-xs';a.href=P+'/d?p='+encodeURIComponent(it.path);a.textContent='Download';
      a.addEventListener('click',function(e){e.stopPropagation();});
      grp.appendChild(a);
    }
...
function renderStats(fc){
  ...
  var q3=document.getElementById('qbar3');if(q3)q3.style.width=pReserved+'%';
  var ql=document.getElementById('qlbl');
  if(ql)ql.textContent=over?('⚠ Used '+fmt(USED)+' + reserved/cache '+fmt(RESERVED+PENDING)+' exceeds capacity '+fmt(QUOTA)+', please delete files'):('Used '+fmt(USED)+' · Reserved '+fmt(rsvDisp)+' · Cache '+fmt(PENDING)+' · Free '+fmt(free)+' · Total '+fmt(QUOTA));
}
...
function del(path,isDir){
  var name=isDir?('Folder "'+path+'" (including everything)'):('「'+path+'」');
  ...
  fetch(P+'/admin/delete',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Deleted '+path);refresh();}else{toast(false,j.error||'Delete failed');}
  }).catch(function(){toast(false,'Delete failed');});
}
function renameItem(path){
  ...
  fetch(P+'/admin/rename',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Renamed');refresh();}else{toast(false,j.error||'Rename failed');}
  }).catch(function(){toast(false,'Rename failed');});
}
function refresh(){
  snapUp();
  fetch(P+'/api/files').then(function(r){return r.json();}).then(function(j){
...
if(bnf){bnf.onclick=function(){
  ...
  var nm=name.trim();
  if(nm.indexOf('/')>=0||nm.indexOf('\\\\')>=0){toast(false,'Folder name cannot contain slashes');return;}
  var p=dir?(dir+'/'+nm):nm;
  ...
  fetch(P+'/admin/mkdir',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Folder created: '+p);refresh();}
    else{toast(false,j.error||'Create failed');}
  }).catch(function(){toast(false,'Create failed');});
};}
var bzip=document.getElementById('selZipBtn');
if(bzip){bzip.onclick=zipDown;}
var bclear=document.getElementById('selClearBtn');
...
function cancelFailed(){
  ...
  if(!failed.length){toast(false,'No failed uploads');return;}
  if(!confirm('Cancel '+failed.length+' failed uploads? Their chunks are kept as cache.'))return;
...
if(bca){bca.onclick=function(){
  if(!queue.length){toast(false,'No uploads in progress');return;}
  if(!confirm('Cancel all '+queue.length+' uploads? Chunks are kept; you can resume later.'))return;
...
if(blog){blog.onclick=function(){
  ...
  }).catch(function(){document.getElementById('logBody').textContent='Load failed';});
};}
...
if(bvc){bvc.onclick=function(){
  ...
  document.getElementById('cacheBody').textContent='Loading…';
  document.getElementById('cacheSum').textContent='';
  fetch(P+'/admin/cache').then(function(r){return r.json();}).then(function(j){
    document.getElementById('cacheSum').textContent='Total '+j.count+' sessions';
    var body=document.getElementById('cacheBody');
    if(!j.items.length){body.innerHTML='<div style="padding:30px;text-align:center;color:var(--muted)">No cache</div>';return;}
    var h='<table class="cache-table"><thead><tr><th>HASH</th><th>name</th><th>chunks</th><th>size</th><th>last active</th></tr></thead><tbody>';
    ...
    h+='</tbody></table>';
    body.innerHTML=h;
  }).catch(function(){document.getElementById('cacheBody').textContent='Load failed';});
};}
...
if(bpurge){bpurge.onclick=function(){
  if(!confirm('Clear all temp chunks of unfinished uploads? (Uploaded files are unaffected)'))return;
  fetch(P+'/admin/upload/clear',{method:'POST'}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Cleared '+j.freed_h+' temp chunks');refresh();}
    else{toast(false,j.error||'Clear failed');}
  }).catch(function(){toast(false,'Clear failed');});
};}
var bpwd=document.getElementById('btnPwd');
if(bpwd){bpwd.onclick=function(){
  var oldp=prompt('Enter the current admin password:');
  if(oldp===null)return;
  var newp=prompt('Enter a new password (non-empty):');
  if(newp===null||newp.length<5){toast(false,'New password cannot be empty');return;}
  ...
  fetch(P+'/admin/password',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Password changed');}else{toast(false,j.error||'Change failed');}
  }).catch(function(){toast(false,'Change failed');});
};}
var bquota=document.getElementById('btnQuota');
if(bquota){bquota.onclick=function(){
  var cur=(QUOTA/1073741824).toFixed(1);
  var gb=prompt('Set max pool capacity (GB, range 0.5 ~ 200):', cur);
  if(gb===null)return;
  var v=parseFloat(gb);
  if(isNaN(v)||v<0.5||v>200){toast(false,'Capacity must be between 0.5 and 200 GB');return;}
  if(v*1073741824 < USED){toast(false,'Target capacity is less than current usage '+fmt(USED)+', please delete some files');return;}
  var fd=new FormData();fd.append('gb',v);
  fetch(P+'/admin/quota',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(j){
    if(j.ok){toast(true,'Pool capacity set to '+j.quota_h);refreshStats();}
    else{toast(false,j.error||'Set failed');}
  }).catch(function(){toast(false,'Set failed');});
};}
renderSel();
render();renderTasks();
</script>"""
...
ADMIN_HTML = """<section class="hero">
  ...
  <div class="acts">
    <a class="btn btn-white btn-sm" href="__PREFIX__/">🏠 Frontend</a>
    <a class="btn btn-ghost btn-sm" href="__PREFIX__/admin/logout">🚪 Logout</a>
    <span class="act-sep"></span>
    <button class="btn btn-multi btn-sm" id="btnViewCache">📋 View cache</button>
    <button class="btn btn-multi btn-sm" id="btnPurge">🧹 Clear cache</button>
    <span class="act-sep"></span>
    <button class="btn btn-multi btn-sm" id="btnQuota">⚖️ Change capacity</button>
    <button class="btn btn-multi btn-sm" id="btnPwd">🔑 Change password</button>
    <span class="act-sep"></span>
    <button class="btn btn-multi btn-sm" id="btnLog">📄 Logs</button>
  </div>
  <div class="stats">
    <div class="stat" id="st-count"><b>-</b><span>files</span></div>
    <div class="stat" id="st-size"><b>-</b><span>total size</span></div>
...
<div class="panel">
  <h3>☁️ Upload <span id="tasksum" style="font-size:12px;color:var(--muted);font-weight:600"></span></h3>
  <div id="taskStatus" class="task-status"></div>
  <div class="zone" id="zone">
    <div class="big">☁️</div>
    <div style="font-weight:700;color:var(--ink);font-size:15px">Drag files or folders here, or click to select</div>
    <div class="tip">3-way chunked upload · resumable</div>
    <div class="acts" style="justify-content:center;margin-top:14px">
      <button class="btn btn-add" id="btnFiles">📄 Select files</button>
      <button class="btn btn-primary" id="btnFolder" style="padding:9px 22px">📁 Select folder</button>
    </div>
  </div>
  ...
  <div class="acts" id="bulkBar" style="justify-content:center;margin-top:14px">
    <button class="btn btn-ghost btn-sm" id="btnPauseAll">⏸ Pause all</button>
    <button class="btn btn-add btn-sm" id="btnResumeAll">▶ Resume all</button>
    <button class="btn btn-add btn-sm" id="btnRetryAll">↻ Retry all</button>
    <button class="btn btn-ghost btn-sm" id="btnCancelFailed">✖ Drop failed</button>
    <button class="btn btn-danger btn-sm" id="btnCancelAll">✕ Cancel all</button>
  </div>
  <div id="tasks" style="margin-top:16px"></div>
  <div class="pager" id="taskPager" style="display:none">
    <button class="btn btn-ghost btn-xs" id="tpPrev">← Prev</button>
    <span id="tpInfo"></span>
    <button class="btn btn-ghost btn-xs" id="tpNext">Next →</button>
    <span class="pg-jump">Go to <input id="tpJump" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="4" page</span><button class="btn btn-ghost btn-xs" id="tpGo">Go</button>
  </div>
</div>
<div class="modal" id="logModal" style="display:none">
  <div class="modal-card">
    <div class="modal-head"><b>📄 Server log (last 300 lines)</b><button class="btn btn-ghost btn-xs" id="logClose">✕ Close</button></div>
    <pre id="logBody">Loading…</pre>
  </div>
</div>
<div class="modal" id="cacheModal" style="display:none">
  <div class="modal-card">
    <div class="modal-head"><b>📋 Upload cache (temp chunks)</b><span id="cacheSum" style="font-size:12px;color:var(--muted);font-family:'Courier New',monospace"></span><button class="btn btn-ghost btn-xs" id="cacheClose">✕ Close</button></div>
    <div class="cache-body" id="cacheBody">Loading…</div>
  </div>
</div>
<nav class="crumbs" id="crumbs"></nav>
<div class="search"><span class="m">🔍</span><input id="q" type="text" placeholder="Search current folder…" autocomplete="off"></div>
...
<div class="selbar" id="selbar" style="display:none">
  ...
  <button class="btn btn-ghost btn-xs" id="selAllBtn">Select all in folder</button>
  <button class="btn btn-primary btn-sm" id="selZipBtn">↓ Download ZIP</button>
  <button class="btn btn-danger btn-sm" id="selDelBtn">🗑 Delete</button>
  <button class="btn btn-ghost btn-sm" id="selClearBtn">Cancel</button>
</div>
"""
...
<div class="auth">
  <div class="auth-card">
    ...
    <a class="btn btn-ghost" href="{U("/")}" style="margin-top:10px">Back to library</a>
  </div>
</div>
""")
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _send_zip(self, items):
        ...
        for rel in items:
            ...
            if p and os.path.exists(p):
                members.append(p)
        if not members:
            self._json({"error": "Nothing to archive"})
            return
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            if "msg" in query:
                v = query["msg"][0]
                msg = ("ok", "Operation succeeded") if v == "ok" else ("err", v)
        ...
        if path == "/d":
            ...
            if not target or not os.path.isfile(target):
                self._send(404, "text/html; charset=utf-8", page("Not Found", '<div class="card" style="padding:40px;text-align:center"><div class="big" style="font-size:40px">🤷</div><p>File not found</p><p style="margin-top:12px"><a class="btn btn-ghost" href="' + U("/") + '">Back to library</a></p></div>').encode("utf-8"))
                return
        ...
        if path == "/admin/log":
            ...
            try:
                ...
                self._json({"ok": True, "log": "(no log file yet)"})
            return
        ...
        self._send(404, "text/html; charset=utf-8", page("404", '<div class="card" style="padding:40px;text-align:center"><p>Page not found</p></div>').encode("utf-8"))
    ...
    def do_POST(self):
        ...
        if path == "/admin/login":
            ...
            if hashlib.sha256(pw.encode()).hexdigest() == ADMIN_PASSWORD_HASH:
                ...
                self._redirect(admin_uri("Wrong password"))
            return
        ...
        if path == "/admin/upload/chunk":
            ...
            meta = upload_meta(uid) if uid else None
            if not meta:
                self._json({"error": "Upload session missing, please reselect the file"}, 400)
                return
            if index < 0 or index >= meta["total_chunks"]:
                self._json({"error": "Chunk out of range"}, 400)
                return
            clen = int(self.headers.get("Content-Length") or 0)
            if clen > CHUNK:
                self._json({"error": "Chunk exceeds the 5MB limit"}, 400)
                return
            data = self.rfile.read(clen)
            expect = chunk_expected_len(meta["size"], meta["total_chunks"], index)
            if len(data) != expect:
                self._json({"error": "Chunk length mismatch, please re-upload this chunk"}, 400)
                return
            sha = query.get("sha256", [""])[0]
            if sha and not re.fullmatch(r"[0-9a-f]{64}", sha):
                self._json({"error": "Invalid checksum parameter"}, 400)
                return
            if sha and hashlib.sha256(data).hexdigest() != sha:
                self._json({"error": "Chunk checksum failed (content mismatch), please re-upload"}, 400)
                return
        ...
        if path == "/admin/upload/init":
            ...
            if not rel or size < 0:
                self._json({"error": "Invalid parameter"})
                return
            target = safe_path(rel)
            if not target:
                self._json({"error": "Illegal path"})
                return
            if os.path.isdir(target):
                self._json({"error": "A folder with the same name exists"})
                return
            ...
            while "/" in cur:
                ...
                if os.path.isfile(safe_path(cur)):
                    self._json({"error": "Cannot write: a file exists at path " + cur})
                    return
            used = pool_used()
            this_need = size
            ...
            if used + committed + this_need > QUOTA:
                self._json({"error": "Not enough capacity: %s left, %s needed" % (human_size(max(0, QUOTA - used - committed)), human_size(size))})
                return
        ...
        if path == "/admin/upload/complete":
            ...
            meta = upload_meta(uid) if uid else None
            if not meta:
                self._json({"error": "Upload session not found"})
                return
            rel = meta["path"]
            size = meta["size"]
            target = safe_path(rel)
            ...
            if missing:
                self._json({"error": "%d chunks not yet uploaded" % len(missing), "missing": missing})
                return
            for i in range(total):
                expect = chunk_expected_len(size, total, i)
                if os.path.getsize(os.path.join(udir, str(i))) != expect:
                    self._json({"error": "Chunk data error, please re-upload"})
                    return
            ...
            try:
                ...
                if os.path.getsize(tmp) != size:
                    self._json({"error": "Merged file size check failed"})
                else:
                    os.replace(tmp, final)
                    ok = True
            except OSError:
                self._json({"error": "Write failed, disk may be full"})
            finally:
                try:
                    os.remove(tmp)
                except OSError:
        ...
        if path == "/admin/password":
            ...
            if hashlib.sha256(old.encode()).hexdigest() != ADMIN_PASSWORD_HASH:
                self._json({"error": "Wrong old password"})
                return
            if not new:
                self._json({"error": "New password cannot be empty"})
                return
        ...
        if path == "/admin/quota":
            try:
                ...
                return
            if not (0.5 <= gb <= 200):
                self._json({"error": "Capacity must be between 0.5 and 200 GB"})
                return
            new_q = int(gb * 1024 * 1024 * 1024)
            used = pool_used()
            if used > new_q:
                self._json({"error": "Currently using %s, exceeds target %s; delete files or set a larger capacity" % (human_size(used), human_size(new_q))})
                return
            set_quota(new_q)
        ...
        if path == "/admin/mkdir":
            ...
            try:
                ...
                self._json({"error": "Create failed"})
            return
        ...
        if path == "/admin/rename":
            ...
            if not os.path.exists(t_old):
                self._json({"error": "Target not found"})
                return
            if os.path.exists(t_new):
                self._json({"error": "A target with the same name exists"})
                return
            try:
                os.rename(t_old, t_new)
                self._json({"ok": True, "path": new})
            except OSError:
                self._json({"error": "Rename failed"})
            return
        ...
        if path == "/admin/delete":
            ...
            if is_dir and os.path.isdir(target):
                try:
                    ...
                    self._json({"error": "Delete failed"})
        ...
        if path == "/admin/batch":
            ...
            op = fl.get("op", [""])[0]
            if op != "delete":
                self._json({"error": "Unknown operation"})
                return
            ...
            for rel in paths:
                target = safe_path(rel)
                if not target:
                    failed.append({"path": rel, "error": "Illegal path"})
                    continue
                try:
                    if os.path.isdir(target):
                        shutil.rmtree(target)
                        ...
                        failed.append({"path": rel, "error": "Target not found"})
                        continue
                    done += 1
                except OSError:
                    failed.append({"path": rel, "error": "Delete failed"})
