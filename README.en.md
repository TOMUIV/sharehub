...
The release ships two UI languages:

- `server.py` — Chinese
- `server-en.py` — English

Pick one when deploying, e.g. English:

```bash
python3 server-en.py --host 0.0.0.0 --port 18888 --prefix ""
```

The Docker image defaults to Chinese. The English build is generated by `build_en.py` from the Chinese source; using `server-en.py` directly needs no regeneration.
